...
export function DailyLogModal({ isOpen, onClose, cultivationId }: DailyLogModalProps) {
    ...
    if (typeof window !== "undefined") {
      ...
      if (saved) {
        ...
        if (idx !== -1 && cultivations[idx].agronomicData) {
          cultivations[idx].agronomicData = cultivations[idx].agronomicData.filter((param: any) => param.date !== date)
  ...
  useEffect(() => {
    if (isOpen && typeof window !== "undefined") {
      ...
      if (saved) {
        ...
        if (found && found.agronomicData) {
          setRecentParams([...found.agronomicData].slice(-5).reverse())
        } else {
          setRecentParams([])
        }
      }
  ...
  const handleSubmit = async () => {
    ...
    const saved = localStorage.getItem("cultivations")
    let cultivations = saved ? JSON.parse(saved) : []
    const idx = cultivations.findIndex((c: any) => c.id === cultivationId)
    if (idx !== -1) {
      const logEntry = {
        ...
        entryType,
        description,
        photo: photo ? photo.name : null, // In real app, upload photo
        details: {},
      }
      if (entryType === "parameter") {
        ...
        cultivations[idx].agronomicData = cultivations[idx].agronomicData || []
        cultivations[idx].agronomicData.push({
          date: logEntry.date,
          ph: details.ph,
          ec: details.ec,
        ...
        cultivations[idx].events = cultivations[idx].events || []
        cultivations[idx].events.push({
          date: logEntry.date,
          type: "action",
        ...
        cultivations[idx].incidents = cultivations[idx].incidents || []
        cultivations[idx].incidents.push({
        ...
        cultivations[idx].events.push({
          ...
          type: "incident",
          description: logEntry.description,
          incidentId: cultivations[idx].incidents[cultivations[idx].incidents.length - 1].id,
        })
      }
      localStorage.setItem("cultivations", JSON.stringify(cultivations))
    }
          ...
          {entryType === "problem" && (
            <>
              ...
              <div className="grid grid-cols-4 items-center gap-4">
                ...
                <Input id="photo" type="file" onChange={handlePhotoChange} className="col-span-3" accept="image/*" />
                {photo && <span className="col-span-3 text-xs text-muted-foreground">{photo.name}</span>}
              </div>
              <div className="grid grid-cols-4 items-center gap-4">
